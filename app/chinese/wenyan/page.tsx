...
export default function ChineseWenyanPage() {
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_12%_8%,rgba(122,61,255,0.10),transparent_38%),radial-gradient(circle_at_88%_10%,rgba(255,174,19,0.12),transparent_36%),linear-gradient(180deg,#fbfaff_0%,#f7f7fb_100%)] px-4 py-6 md:px-6">
          <div className="mx-auto w-full max-w-4xl">
            {!selectedText ? (
              <TextPicker onSelect={selectText} />
            ) : (
...
function TextPicker({ onSelect }: { onSelect: (id: string) => void }) {
  return (
    <div className="space-y-6">
      <div className="space-y-2 text-center">
        ...
        <p className="mx-auto max-w-xl text-sm leading-7 text-[#6b6385]">
          選擇一篇文言文，跟着四個任務一步步學：先讀通原文，再拆解句子成分，然後試着翻譯並由 AI 檢測，最後對照完整白話翻譯。
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {WENYAN_TEXTS.map((t, i) => (
          <button
            ...
            className="group flex flex-col rounded-[16px] border border-[#e6e1f3] bg-white/90 p-5 text-left shadow-[0_10px_28px_rgba(122,61,255,0.08)] transition duration-200 hover:-translate-y-1 hover:border-[#7a3dff]/50"
          >
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 pl-9">
                ...
                {err && (
                  <p className="mt-2 text-sm text-[#c21830]">{err}</p>
                )}
...
function Task4({ text }: { text: WenyanText }) {
  return (
    <div className="space-y-4">
      <div className="rounded-[14px] border border-[#e6e1f3] bg-[#faf8ff] p-4 text-sm leading-7 text-[#5b4f7a]">
        恭喜你完成翻譯練習！下面是原文和完整的白話翻譯，把它和你自己的翻譯對照一下，看看哪裏可以譯得更好。
      </div>

...
const modes = [
  {
    ...
    description:
      "Learn scanning, inference and text-type skills through the 'Make a Balloon Puff Up' experiment sheet and step-by-step practice.",
    icon: BookOpen,
    color: "#ff6b00",
    available: true,
  },
...
export default function EnglishReadingComprehensionCycle3Reading2ModesPage() {
  const router = useRouter();

  function navigateToMode(modeId: string, available: boolean) {
    if (!available) return;
    if (modeId === "roleplay") {
      router.push("/english/reading-comprehension/cycle-3-reading-2/roleplay");
      return;
    }
    if (modeId === "learning") {
      router.push("/english/reading-comprehension/cycle-3-reading-2/learning");
    }
  }
          ...
          <div className="flex w-full flex-col gap-10 py-2">
            <section className="space-y-3 px-2 sm:px-0">
              <p className="text-[12px] font-semibold uppercase tracking-[1.2px] text-[#ababab]">
                Cycle 3 · Reading 2 — Make a Balloon Puff Up (Experiment)
              </p>

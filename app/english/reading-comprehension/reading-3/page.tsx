...
const modes = [
  {
    ...
    description:
      "Learn skimming, scanning, inference and fact-vs-opinion skills through the story of Pip the dragon and step-by-step practice.",
    icon: BookOpen,
    color: "#ff6b00",
    available: true,
  },
...
export default function EnglishReadingComprehensionReading3ModesPage() {
  const router = useRouter();

  function navigateToMode(modeId: string, available: boolean) {
    if (!available) return;
    if (modeId === "roleplay") {
      router.push("/english/reading-comprehension/reading-3/roleplay");
      return;
    }
    if (modeId === "learning") {
      router.push("/english/reading-comprehension/reading-3/learning");
    }
  }
          ...
          <div className="flex w-full flex-col gap-10 py-2">
            <section className="space-y-3 px-2 sm:px-0">
              <p className="text-[12px] font-semibold uppercase tracking-[1.2px] text-[#ababab]">
                Cycle 1 · Reading 3 — Pip the Dragon (Story)
              </p>

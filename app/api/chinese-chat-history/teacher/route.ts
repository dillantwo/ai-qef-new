...
// The three Chinese topics this viewer currently supports.
const CHINESE_TOPICS = ["scenery-description", "character-description", "lin-zexu"];
  ...
  try {
    ...
    if (studentId) {
      ...
      const docs = await ChineseChatHistory.find({
        userId: studentId,
        topic: topic && CHINESE_TOPICS.includes(topic) ? topic : { $in: CHINESE_TOPICS },
      })
        .sort({ updatedAt: -1 })
        .lean();
    ...
    console.error("[chinese-chat-history/teacher] GET Error:", error);

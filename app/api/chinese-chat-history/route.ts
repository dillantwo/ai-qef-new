...
export async function GET(req: Request) {
  try {
    ...
    if (chatId) {
      const doc = await ChineseChatHistory.findOne({ userId: session.userId, chatId }).lean();
      if (!doc) {
    ...
    console.error("[chinese-chat-history] GET Error:", error);
    return new Response(
...
export async function POST(req: Request) {
  try {
    ...
    const { id, title, topic, messages } = (await req.json()) as {
      id?: string;
      title?: string;
      topic?: string;
    ...
    const doc = await ChineseChatHistory.findOneAndUpdate(
    ...
    console.error("[chinese-chat-history] POST Error:", error);
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : "Unknown error" }),
      { status: 500, headers: { "Content-Type": "application/json" } },
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    await connectDB();
    await ChineseChatHistory.deleteOne({ userId: session.userId, chatId });
    return Response.json({ ok: true });
  } catch (error) {
    console.error("[chinese-chat-history] DELETE Error:", error);
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : "Unknown error" }),
      { status: 500, headers: { "Content-Type": "application/json" } },
    );
